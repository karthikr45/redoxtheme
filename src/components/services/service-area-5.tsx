...
const ServiceAreaFive = ({ spacing = "section-spacing-top", title_font = "font-bdogrotesk-regular", data }: IProps) => {
  const subtitle = data?.subtitle || "Services";
  const heading = data?.heading || "Services we provide";
  const description = data?.description || "We are here to build solid and courageous brands that can leave a strong mark on the world.";
          ...
          <div className="services-wrapper-box">
            ...
            <div className="services-wrapper-5">
              {items.map((service, idx) => (
                <a href={service.link} key={idx}>
                  <div className="service-box fade-anim">
                    <div className="count"><span className="number">{service.number}</span></div>
                    <div className="content">
                      <h3 className="title">{service.title}</h3>
                      <p className="text">{service.text}</p>
                    </div>
                    <div className="thumb">
                      <Image src={service.image} alt={service.title} width={165} height={92} />
                    </div>
                  </div>
                </a>
              ))}
